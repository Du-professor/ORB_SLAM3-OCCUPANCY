...
> 默认 launch 会同时启动：
> - `orb_slam3_main` 主程序（RGB-D SLAM）
> - `occupancy_grid_map`（消费 `/slam/filtered_cloud`，输出 2D 栅格 `slam/occupancy_grid`）
> - `rviz2`（加载 `/home/test/robot_ws/install/orbslam3_dense_ros2/share/orbslam3_dense_ros2/config/rviz_config.rviz`）

如果需要自定义参数，可通过 `ros2 launch orbslam3_dense_ros2 slam_system.launch.py <param:=value>` 调整，例如：

```shell
ros2 launch orbslam3_dense_ros2 slam_system.launch.py \
	rgb_topic:=/camera/color/image_raw \
	depth_topic:=/camera/aligned_depth_to_color/image_raw \
	filtered_voxel_leaf_size:=0.1 \
	occupancy_config:=/path/to/occupancy_grid_map.param.yaml \
