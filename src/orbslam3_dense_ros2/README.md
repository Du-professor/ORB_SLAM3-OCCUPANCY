...
## Filtered point cloud & occupancy grid

`ros2_slam_publisher` can now publish an occupancy-ready, downsampled cloud while it streams poses and map points. The new parameters (exposed via the launch file) let you tune voxel size, XY/Z limits, and the publication rate:

- `filtered_cloud_topic` (default `/slam/filtered_cloud`)
- `filtered_voxel_leaf_size` (meters)
- `filtered_min_* / filtered_max_*` for range and height gating
- `filtered_cloud_frame` to pick the frame seen by downstream consumers

Launch `slam_system.launch.py` with `enable_occupancy:=true` to spin up the revamped `occupancy_grid_map` node, which converts the filtered cloud into a log-odds 2D grid (published on `slam/occupancy_grid`). The occupancy node accepts the same YAML you can run standalone (`ros2 launch occupancy_grid_map occupancygrid.launch.py`).
