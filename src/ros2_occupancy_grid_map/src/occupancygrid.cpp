...
namespace PerceptionNS
{
        ...
        pointcloud_sub2_(this, "input/pcl_2", rclcpp::SensorDataQoS().get_rmw_qos_profile())
  {
    ...
    use_dual_input_ = this->declare_parameter<bool>("use_dual_input", false);
    incremental_update_ = this->declare_parameter<bool>("incremental_update", true);
    min_height_filter_ = this->declare_parameter<double>("min_z", -0.3);
    max_height_filter_ = this->declare_parameter<double>("max_z", 2.0);
    ...
    max_queue_size_ = this->declare_parameter<int>("processing_queue_size", 4);
    auto pointcloud_topic = this->declare_parameter<std::string>("pointcloud_topic", "cloudPointMapping");
  ...
  grid_template_.assign(grid_size_, 50);
  grid_msg_.data = grid_template_;
    ...
    if (use_dual_input_)
    {
      ...
      sync_->registerCallback(std::bind(&GroundSegmentor::enqueue_pointcloud_pair, this, _1, _2));
    }
    else
    {
      single_pointcloud_sub_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
          pointcloud_topic,
          rclcpp::SensorDataQoS(),
          std::bind(&GroundSegmentor::single_pointcloud_callback, this, _1));
    }
    ...
    // Spin up processing thread once all interfaces are ready
    processing_running_.store(true);
    processing_thread_ = std::thread(&GroundSegmentor::processing_loop, this);
  }

  GroundSegmentor::~GroundSegmentor()
  {
  ...
  void GroundSegmentor::enqueue_pointcloud_pair(
      ...
      const sensor_msgs::msg::PointCloud2::ConstSharedPtr &input_pcl_msg_2)
  {
    ...
    CloudWithOrigin transformed_first = transformPointCloudToTargetFrame(input_pcl_msg_1);
    CloudWithOrigin transformed_second;
    if (input_pcl_msg_1 == input_pcl_msg_2)
    {
      transformed_second = transformed_first;
    }
    else
    {
      transformed_second = transformPointCloudToTargetFrame(input_pcl_msg_2);
    }

    if (!transformed_first.cloud || !transformed_second.cloud)
    {
      return;
    }

    CloudPair pair{transformed_first, transformed_second};

    {
      std::lock_guard<std::mutex> lock(queue_mutex_);
      if (max_queue_size_ > 0 && static_cast<int>(cloud_queue_.size()) >= max_queue_size_)
      {
        cloud_queue_.pop_front();
      }
      cloud_queue_.push_back(pair);
    }

    queue_cv_.notify_one();
  }

  void GroundSegmentor::single_pointcloud_callback(
      const sensor_msgs::msg::PointCloud2::ConstSharedPtr &input_pcl_msg)
  {
    enqueue_pointcloud_pair(input_pcl_msg, input_pcl_msg);
  }

  void GroundSegmentor::processing_loop()
  {
    while (rclcpp::ok() && processing_running_.load())
    {
      CloudPair pair;
      {
        std::unique_lock<std::mutex> lock(queue_mutex_);
        queue_cv_.wait(lock, [this]() { return !cloud_queue_.empty() || !processing_running_.load(); });
        ...
        pair = cloud_queue_.front();
        cloud_queue_.pop_front();
      }

      process_pointcloud_pair(pair);
    }
  }

  GroundSegmentor::CloudWithOrigin GroundSegmentor::transformPointCloudToTargetFrame(
      const sensor_msgs::msg::PointCloud2::ConstSharedPtr &input_msg)
  {
    CloudWithOrigin result;
    if (!input_msg)
    {
      return result;
    }

    auto transformed = std::make_shared<sensor_msgs::msg::PointCloud2>(*input_msg);

    try
    {
      auto tf = tf_buffer_.lookupTransform(target_frame_, input_msg->header.frame_id, tf2::TimePointZero);

      if (input_msg->header.frame_id != target_frame_)
      {
        tf2::doTransform(*input_msg, *transformed, tf);
        transformed->header.frame_id = target_frame_;
      }

      result.cloud = transformed;
      result.origin_x = tf.transform.translation.x;
      result.origin_y = tf.transform.translation.y;
      return result;
    }
  ...
  pcl::PointCloud<pcl::PointXYZI>::Ptr GroundSegmentor::FilterIncomingCloud(
      const pcl::PointCloud<pcl::PointXYZI>::Ptr &cloud) const
  {
    ...
    pass.setFilterLimits(static_cast<float>(min_height_filter_), static_cast<float>(max_height_filter_));
    pass.filter(*filtered);

    if (voxel_leaf_size_ > 0.0 && !filtered->empty())
    {
  ...
  void GroundSegmentor::process_pointcloud_pair(const CloudPair &pair)
  {
    if (!pair.raw.cloud || !pair.obstacle.cloud)
    {
      return;
    }

    auto raw_cloud = std::make_shared<pcl::PointCloud<pcl::PointXYZI>>();
    auto obstacle_cloud = std::make_shared<pcl::PointCloud<pcl::PointXYZI>>();
    pcl::fromROSMsg(*pair.raw.cloud, *raw_cloud);
    pcl::fromROSMsg(*pair.obstacle.cloud, *obstacle_cloud);

    raw_cloud->is_dense = false;
    obstacle_cloud->is_dense = false;

    raw_cloud = FilterIncomingCloud(raw_cloud);
    obstacle_cloud = FilterIncomingCloud(obstacle_cloud);
    ...
    if (!obstacle_cloud || obstacle_cloud->empty())
    {
      obstacle_cloud = raw_cloud;
    }
    ...
    std::vector<int> ind;
    pcl::removeNaNFromPointCloud(*raw_cloud, *non_nan_pcl, ind);

    if (non_nan_pcl->empty())
    {
      return;
    }
    ...
    pcl::removeNaNFromPointCloud(*obstacle_cloud, obs_pcl, obs_indices);
    if (obs_pcl.empty())
    {
      obs_pcl = *obstacle_cloud;
    }
    ...
    if (grid_msg_.data.empty())
    {
      grid_msg_.data = grid_template_;
    }
    else if (!incremental_update_)
    {
      grid_msg_.data = grid_template_;
    }

    // Apply raytracing (free space from sensor origin to latest return)
    for (size_t j = 0; j < angle_bin_size - 1; j++)
    {
      ...
      raytrace(grid_msg_, pair.raw.origin_x, pair.raw.origin_y, source.wx, source.wy, cell_num_x_, CellState::FREE);
    }
  ...
  grid_msg_.header.set__stamp(pair.raw.cloud->header.stamp);
    grid_msg_.header.frame_id = target_frame_;
